...
class App(tk.Frame):           
    def __init__(self, master):
        ...
        self.input_bar.pack(side='top',  fill=tk.X,  padx=5,  pady=5,  expand= True)
        # self.input_bar.pack_propagate(0)
        tk.Label(self.input_bar, text="Input data (.csv):                                  ").pack( fill=tk.X)
        ...
        tk.Checkbutton(self.log_bar, text="log2 re-scale values", variable=self.rescaled, onvalue=False, offvalue=True, height=1, width=15).pack(side=tk.LEFT,padx=5,pady=0, fill=tk.X)
        ...
        self.curve_params = tk.BooleanVar()
        tk.Checkbutton(self.data_bar, text="Use pre-determined parameters:    ", variable=self.curve_params, onvalue=True, offvalue=False, height=1, width=25).pack(padx=5,pady=0, fill=tk.X)
        ...
        self.input_bar.pack(side='top',  fill=tk.X,  padx=5,  pady=5,  expand= False)
        ...
        tk.Checkbutton(self.filter_bar, text="weighted (slow mode)", variable=self.weighted, onvalue=True, offvalue=False, height=1, width=15).pack(padx=5,pady=5, fill=tk.X)
        
        self.pro_bar  =  tk.Frame(self.filter_bar,  width=15,  height=1)
        self.pro_bar.pack(side='top',  fill=tk.X,  padx=5,  pady=5,  expand= False)
    ...
    def open_edge(self):
        self.opend_edge_file = tk.filedialog.askopenfilename()
    ...
    def clicked(self):
        ...
        if not os.path.exists(self.data.get()):
            self.mylist.insert(tk.END, '--Data not found, check the input data.')
            return
        ...
        if self.save_data.get():
            self.mylist.insert(tk.END, '--The normalzied dataset is saved as: z-scored '+self.data.get())
        
        self.mylist.insert(tk.END, '--Calculating PCCs...')
        gene.Calculate_PCC(pcc_path = self.pcc.get() )          
        if self.pcc.get():
            self.mylist.insert(tk.END, '--The PCC matrix is saved as: PCC_matrix.csv')
        if self.stopflag:
            self.stopset()
            return  
        ...
        if self.edgelist.get():
            self.mylist.insert(tk.END, '--Edge list is saved as: edgelist.csv')
        if self.thres_curve.get():
            self.mylist.insert(tk.END, '--The threshold curve is saved as: threshold_curve.png')
            img = tk.PhotoImage(file="threshold_curve.png")
    ...
    def NetAttr(self):
        ...
        netw.find_pos(core = self.core.get(), louvain_community = self.community.get())    
        if self.stopflag:
            self.stopset()
            return   
        ...
        if self.closeness.get():
            ...
            netw.netbet(closeness = self.closeness.get())            
            if self.stopflag:
                self.stopset()
                return 
        ...
        if self.community.get() or self.degree.get() or self.core.get() or self.eigenvector.get() or self.betweenness.get() or self.closeness.get():
            netw.saveattr()
            self.mylist.insert(tk.END, '--Property Table is saved as: property_table.csv')

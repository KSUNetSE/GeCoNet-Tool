...
class GeneCoExp():
    ...
    def Calculate_PCC(self, min_periods=4,  pcc_path=False, paired_elements_path=False):
        ...
        if self.corr:
            ...
            if pcc_path:
                self.rho.to_csv('PCC_matrix.csv', sep=',')
                print('PCC table is saved as: PCC_matrix.csv')
    ...
    def paired_elements(self, paired_elements_path = False):
        if self.corr:
            ...
            if paired_elements_path:
                self.paired_items.to_csv('paired_elements_matrix.csv', sep =',')
                print('paired element table is saved as: paired_elements_matrix.csv')
    ...
    def curve_fitting(self, alpha=1,eta=1.5,lam=2,beta=30, para_in= False, edgelist=True, thresholdcurve=True):
        '''
        Parameters
        ----------
        p0 : list, required
        ...
        if edgelist:
            ...
            el.to_csv('edgelist.csv', sep=',', index=False)
            print('edge list is saved as: edgelist.csv')
